...
const BOSSES := [
	preload("res://elements/targets/boss_target/boss_target.tscn")
]
const BOSS_LEVEL := 5
...
func load_game():
	...
	for veriable in SAVE_VARIABLES:
		if veriable in game_data:
			set(veriable, game_data[veriable])
