...
func _ready():
	Events.active_knife_change.connect(update_knife_status)
...
func initialize(knife_idx: int):
	...
	elif Globals.is_knife_unlock(knife_idx):
		change_state(State.OPENED)

func unlock():
	change_state(State.OPENED)

func update_knife_status(active_knife_idx):
	if knife_index == active_knife_idx:
		change_state(State.ACTIVE)
	elif Globals.is_knife_unlock(knife_index):
		change_state(State.OPENED)
	
func _on_button_pressed():
	if not Globals.is_knife_unlock(knife_index):
		return
